...
class Net(pl.LightningModule):
    def __init__(self,bs,Height,Depth,epoch_num,l_rate=1e-3,ku=False, dropout=None, **kwargs):
        super().__init__()
        if ku:
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self._model.parameters(), lr = l_rate, weight_decay=0.0001)
        return optimizer
    ...
    def validation_step(self,batch,batch_idx):
        ...
        sw_batch_size = 1
        outputs = sliding_window_inference(images, roi_size, sw_batch_size, self.foward, overlap=0.5)
    ...
    def test_step(self,batch,batch_idx):
        ...
        sw_batch_size = 1
        test_image = images
        test_output = sliding_window_inference(test_image, roi_size, sw_batch_size, self.foward,overlap=0.5)
...
train_data, val_data = partition_dataset(data_dicts, ratios = [0.95, 0.05], shuffle = True)
...
# generate pl module
net = Net(bs,Height,Depth,epoch_num,l_rate,ku,dropout)
...
trainer.test(net, test_dataloaders=val_loader)