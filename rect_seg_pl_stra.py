...
#test start
# print(f"{network}"+"-{epoch}-{mean_val_loss:.2f}-{mean_val_metric:.2f}-Ver."+f"{version}")
trainer.test(model=net, ckpt_path= 'best', datamodule=dm)
# trainer.test(model=net, ckpt_path=f"{network}"+"-{epoch}-{mean_val_loss:.2f}-{mean_val_metric:.2f}-Ver."+f"{version}", datamodule=dm)
# ckpt_path = checkpoint_callbac -> filename 
