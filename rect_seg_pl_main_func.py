...
class RectalCaSegNet(pl.LightningModule):
    ...
    def __init__(self,bs,Height,Depth,epoch_num,l_rate=1e-3,network='resunet',drop=None,chan=32,**kwargs):
        super().__init__()
        if network=="deeplab":
            ...
            in_channels=1,
            out_channels=2,
            channels=(chan, chan*2, chan*4, chan*8, chan*16),
            strides=(2, 2, 2, 2),
            num_res_units=8,
            norm=Norm.BATCH,
            dropout=drop)
            ...
            self._model = UNet_3Plus_3D(in_channels=1, n_classes=2, feature_scale=4, is_deconv=True, is_batchnorm=True, channels=chan)
...
# # GLOBAL variable? (in class RectalCaSegNet initializer)
# model_channels = 32
...
# net = RectalCaSegNet(bs,Height,Depth,epoch_num,l_rate,network,dropout,model_channels)
